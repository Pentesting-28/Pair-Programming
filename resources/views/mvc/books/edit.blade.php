<x-layouts::mvc :title="__('Editar Libro (MVC)')">
    <flux:container max-width="lg">
        <div class="mb-8">
            ...
            <div class="flex flex-col gap-1">
                <flux:heading size="xl" level="1" class="font-bold tracking-tight text-zinc-900 dark:text-white">Editar Libro</flux:heading>
                <flux:subheading class="text-zinc-500 dark:text-zinc-400">Actualiza la información del ejemplar: <span class="font-semibold text-zinc-900 dark:text-white">{{ $book->title }}</span></flux:subheading>
            </div>
        </div>
        ...
        <flux:card class="p-0 overflow-hidden shadow-sm">
            ...
            <form action="{{ route('mvc.books.update', $book) }}" method="POST" class="flex flex-col">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 space-y-6">
                    ...
                    <flux:field>
                        ...
                        <flux:input name="title" value="{{ old('title', $book->title) }}" required placeholder="Ej. Cien años de soledad" />
                        @error('title')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400 font-medium">{{ $message }}</p>
                        @enderror
                    </flux:field>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <flux:field>
                            ...
                            <flux:input name="isbn" value="{{ old('isbn', $book->isbn) }}" required placeholder="Ej. 978-3-16-148410-0" />
                            @error('isbn')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400 font-medium">{{ $message }}</p>
                            @enderror
                        </flux:field>
                        ...
                        <flux:field>
                            ...
                            <flux:input type="number" name="num_pages" value="{{ old('num_pages', $book->num_pages) }}" required placeholder="Ej. 432" min="1" />
                            @error('num_pages')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400 font-medium">{{ $message }}</p>
                            @enderror
                        </flux:field>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var initialAuthor = @json([old('author_id', $book->author_id)]).filter(Boolean);
