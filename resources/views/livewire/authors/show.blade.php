...
new #[Layout('layouts.livewire')] #[Title('Detalle de Autor')] class extends Component
{
    public Author $author;

    public function mount(Author $author)
    {
        $this->author = $author->load('books', 'country');
    }
}; ?>
...
<flux:container max-width="lg">
    <div class="mb-8">
        <div class="flex items-center gap-2 mb-4">
            <flux:button variant="subtle" size="sm" icon="arrow-left" class="text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200" :href="route('livewire.authors.index')" wire:navigate>
                Volver al listado
            </flux:button>
        </div>
    ...
    <div class="mt-12 flex items-center justify-between pt-6 border-t border-zinc-200 dark:border-zinc-700">
        <flux:button variant="ghost" :href="route('livewire.authors.index')" class="text-zinc-500" wire:navigate>
            Cerrar vista
        </flux:button>
        <div class="flex gap-2">
            <flux:button variant="subtle" icon="pencil" :href="route('livewire.authors.edit', $author)" wire:navigate>
                Editar Perfil
            </flux:button>
        </div>
    </div>
